...
interface IRitualLSTv2 {
    function stake() external payable;
}

contract RitualLSTv2 {
    ...
    uint256 public weeklyYieldBPS = 100;
    bool public manualYieldMode = true;
    bool public isPaused = false;
    bool public sbtGateEnabled = true;
    ...
    uint256 public constant MIN_REBASE_INTERVAL = 1200;
    ...
    event XRitualTokenUpdated(address indexed oldToken, address indexed newToken);
    event YieldWithdrawn(uint256 amount);
    event StakingPaused();
    event StakingUnpaused();
    event MigrationTargetSet(address indexed target);
    event Migrated(address indexed user, uint256 ritualAmount);
    event ManualYieldModeSet(bool enabled);
    event WeeklyYieldBPSUpdated(uint256 oldBPS, uint256 newBPS);
    event SbtGateUpdated(bool enabled);
    ...
    receive() external payable {
        revert("use stake() or depositYield()");
    }

    function stake() external payable nonReentrant whenNotPaused returns (uint256 xRitualMinted) {
        if (sbtGateEnabled) {
    ...
    function unstake(
        uint256 xRitualAmount
    ) external nonReentrant whenNotPaused returns (uint256 ritualReturned) {
    ...
    function applyRebase(uint256 yieldBPS) external onlyOwner {
        require(
            block.number >= lastRebaseBlock + MIN_REBASE_INTERVAL,
            "rebase too soon"
        );
        _applyYield(yieldBPS);
    }
    ...
    function depositYield() external payable onlyOwner {
        ...
        uint256 supply = xRitualToken.totalSupply();
        require(supply > 0, "no stakers");
        yieldReserve += msg.value;
        exchangeRate = ((totalStaked + yieldReserve) * 1e18) / supply;
        emit Rebased(exchangeRate, msg.value, block.number);
    }

    function withdrawYield(uint256 amount) external onlyOwner {
        require(amount > 0, "zero amount");
        require(amount <= yieldReserve, "exceeds yield reserve");
        yieldReserve -= amount;
    ...
    function _applyYield(uint256 yieldBPS) internal {
        ...
        uint256 actualYield = yieldReserve;
        uint256 expectedYield = (totalStaked * yieldBPS) / 10000;
        if (actualYield > expectedYield) {
            actualYield = expectedYield;
        }
        uint256 feeAmount = (actualYield * PROTOCOL_FEE_BPS) / 10000;
        uint256 netYield = actualYield - feeAmount;
        ...
        if (netYield > 0) {
            ...
            yieldReserve -= netYield;
        }

        if (xRitualToken.totalSupply() > 0) {
            exchangeRate =
                ((totalStaked + yieldReserve) * 1e18) /
    ...
    function setXRitualToken(address _xRitualToken) external onlyOwner {
        require(_xRitualToken != address(0), "zero address");
        address oldToken = address(xRitualToken);
        xRitualToken = IxRITUAL(_xRitualToken);
        emit XRitualTokenUpdated(oldToken, _xRitualToken);
    }

    function setSbtGateEnabled(bool enabled) external onlyOwner {
        sbtGateEnabled = enabled;
        emit SbtGateUpdated(enabled);
    }
    ...
    function migrateToV2() external nonReentrant {
        require(migrationTarget != address(0), "no migration target");
        uint256 xBalance = xRitualToken.balanceOf(msg.sender);
        require(xBalance > 0, "nothing to migrate");
        require(unbondingRequests[msg.sender].amount == 0, "claim pending unstake first");

        uint256 ritualAmount = (xBalance * exchangeRate) / 1e18;
        require(address(this).balance >= ritualAmount, "insufficient liquidity");

        xRitualToken.burn(msg.sender, xBalance);
        totalStaked = ritualAmount >= totalStaked
            ? 0
            : totalStaked - ritualAmount;
        ...
        IRitualLSTv2(migrationTarget).stake{value: ritualAmount}();
        emit Migrated(msg.sender, ritualAmount);
    }

    function setWeeklyYieldBPS(uint256 bps) external onlyOwner {
        require(bps <= 1000, "max 10% weekly");
        uint256 oldBPS = weeklyYieldBPS;
        weeklyYieldBPS = bps;
        emit WeeklyYieldBPSUpdated(oldBPS, bps);
    }

    function projectedWeeklyReward(address user) external view returns (uint256) {
        uint256 xBalance = xRitualToken.balanceOf(user);
        if (xBalance == 0) return 0;
        uint256 ritualValue = (xBalance * exchangeRate) / 1e18;
        return (ritualValue * weeklyYieldBPS) / 10000;
    }

    function setManualYieldMode(bool enabled) external onlyOwner {
        manualYieldMode = enabled;
        emit ManualYieldModeSet(enabled);
    }
